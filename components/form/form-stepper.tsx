...
export function FormStepper() {
  ...
  // Build a snapshot from <form> values
  const getFormSnapshot = useCallback(() => {
    ...
    // Support duplicate keys (checkbox groups, multi-select)
  ...
  // Restore snapshot into form inputs
  ...
  const saveDraftNow = useCallback(
    ...
    [currentStep, getFormSnapshot, user]
  );
  ...
  // Autosave on any input change (capture phase)
  useEffect(() => {
    const formEl = formRef.current?.closest("form");
    if (!formEl) return;
  ...
  const submitApplication = useCallback(async () => {
    ...
    try {
      const res = await fetch("/application/submit", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({}), // optional applicationId could go here
      });
      ...
      // Optional: route to confirmation page
      // window.location.href = `/application/confirmation?id=${data.applicationId}`;
      setSavedPulse(true);
      window.setTimeout(() => setSavedPulse(false), 1200);
    } catch (e: any) {
    ...
    <div ref={formRef} className="flex flex-col gap-6">
      ...
      {hasRemoteDraft && remoteDraft && (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/20 bg-primary/5 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-foreground">You have a saved draft in the database. Restore it?</span>
          </div>
          ...
          <div className="flex gap-2 shrink-0">
            ...
            <Button size="sm" onClick={restoreRemoteDraft} className="bg-primary text-primary-foreground text-xs hover:bg-primary/90">
              Restore
            </Button>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4 sm:p-6">
        ...
        <div className="mt-3 flex flex-col items-center justify-center gap-1 text-[11px] text-muted-foreground">
          {isSaving ? <span>Saving…</span> : savedPulse ? <span className="text-primary">Saved</span> : <span>Autosave enabled</span>}
          ...
          <>
            ...
            <div className="flex flex-col gap-2 border border-foreground/30 p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Questions: Please visit our website at <span className="font-medium text-foreground">dmv.dc.gov</span> or call 311 in DC or 202.737.4404 outside the 202 area code.
              </p>
            </div>
          </>
        )}
      ...
      <div className="flex items-center justify-between rounded-lg border border-border bg-card p-4">
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="ghost" size="sm" onClick={() => void saveDraftNow()} className="gap-1.5 text-muted-foreground hover:text-foreground">
            Save now
          </Button>
        </div>
